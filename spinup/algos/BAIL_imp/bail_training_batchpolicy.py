...
class BAIL_selebah(object): # selection in mini-batch
	...
	def train(self, replay_buffer, done_training_iters, iterations=5000, batch_size=1000,
		...
		bail_state_dict = {'policy_net': self.actor.state_dict(), 'v_ue_net': self.v_ue.state_dict(),
						  'best_v_ue_net': self.best_v_ue.state_dict()}
		return bail_state_dict
...
def plot_envelope(upper_envelope, states, returns, setting, seed, hyper_lst, make_title=False):
	...
	plt.tight_layout()
	plt.savefig('./plots/' + "ue_visual_%s.png" % setting.replace('.', '-'))
	plt.close('all')
